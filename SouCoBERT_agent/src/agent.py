...
def initialize():
    ...
    model.load_state_dict(tc.load("./src/model_weights.pth", map_location=tc.device('cpu')))
...
def decompile_process(code, return_dict):
    try:
        return_dict['result'] = preprocess_code(simple_web3_code_decompile(code))
    except:
...
def detect_malicious_contract(w3, from_, created_contract_address) -> list:
    ...
    if created_contract_address is not None:
        ...
        code = w3.eth.get_code(Web3.toChecksumAddress(created_contract_address)).hex()
        p = multiprocessing.Process(target=decompile_process, args=(code, return_dict))
        p.start()
        p.join(30)
        if p.is_alive():
            logger.info("decompilation deadline exceeded")
            p.terminate()
            p.join()
            decompiled = "error ;"
        else:
            decompiled = return_dict['result']

        if "error" in decompiled or len(decompiled) < 100:
            findings.append(
                MaliciousContractFindings.unable_to_decompile(
                    from_,
                    created_contract_address,
                )
            )
        ...
        if y == 1:
            print(created_contract_address)
